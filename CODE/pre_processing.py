    ...
    def pre_processing(self, df :pd.DataFrame, train : bool, categorical_var_OHE:list,
        ...
        if train == True :
            ...
            if len(continious_var) != 0 :
                ...
                df[continious_var] = self.scaler.fit_transform(df[continious_var])    
           
            #categorical encoding :
            if len(categorical_var_OHE) != 0 :
            #categorical var : OHE
                self.enc_OHE = OneHotEncoder(drop='first', sparse=False).fit(df[categorical_var_OHE])
                encoded = self.enc_OHE.transform(df[categorical_var_OHE])
                encoded_df = pd.DataFrame(encoded,columns=self.enc_OHE.get_feature_names_out())
                df.drop(categorical_var_OHE, axis=1, inplace=True)
            ...
            #categorical var : Ordinal input example -> {"var" : {'c':0,'b':1,'a':2}}
                for i in range(len(categorical_var_OrdinalEncoding)) :
                    ...
                    df[var] = self.enc_ordinal.fit_transform(df[var])
            
            if len(categorical_var_TE) != 0 :
            #categorical var : Target encoding
                self.enc_TE = TargetEncoder().fit(df[categorical_var_TE], target)
                encoded_TE = self.enc_TE.transform(df[categorical_var_TE])
                encoded_df_TE = pd.DataFrame(encoded_TE,columns=self.enc_TE.get_feature_names_out())
                df.drop(categorical_var_TE, axis=1, inplace=True)
                df = pd.concat([df, encoded_df_TE], axis=1)
            ...
            if len(continious_var) != 0 :
                #continious var :
                df[continious_var] = self.scaler.transform(df[continious_var])
             
            #categorical encoding :  
            if len(categorical_var_OHE) != 0 :
                #categorical var : OHE
                encoded2 = self.enc_OHE.transform(df[categorical_var_OHE])
            ...
            #categorical var : Ordinal input example -> {"var" : {'c':0,'b':1,'a':2}}
                for i in range(len(categorical_var_OrdinalEncoding)) :
                    var = list(categorical_var_OrdinalEncoding.keys())[i]    
                    df[var] = self.enc_ordinal.transform(df[var])
             
            if len(categorical_var_TE) != 0 :
                #categorical var : Target Encoding
                encoded2_TE = self.enc_TE.transform(df[categorical_var_TE])
                encoded_df2_TE = pd.DataFrame(encoded2_TE,columns=self.enc_TE.get_feature_names_out())
                df.drop(categorical_var_TE, axis=1, inplace=True)
                df = pd.concat([df, encoded_df2_TE], axis=1)
